...
final class WorkoutModeController extends Controller
{
    ...
    public function switchExercise(): void { header('Content-Type: application/json');try{echo json_encode($this->service->switchExercise((int)($_POST['workout_exercise_id']??0)),JSON_THROW_ON_ERROR);}catch(RuntimeException $e){http_response_code(422);echo json_encode(['error'=>$e->getMessage()]);} }
    ...
    public function finishEarly(): void { try{$this->service->finishEarly();$this->redirect(route('workouts.active.summary'));}catch(RuntimeException $e){$this->redirect(route('workouts.active'),$e->getMessage());} }
