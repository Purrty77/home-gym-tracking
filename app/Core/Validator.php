...
final class Validator
{
    ...
    {
        $errors=[];
        if(empty($data['performed_at'])||!strtotime($data['performed_at']))$errors[]='The workout date is invalid.';
        if(trim((string)($data['session_type']??''))==='')$errors[]='The workout type is required.';
        if(($data['body_weight_kg']??'')!==''&&!self::number($data['body_weight_kg'],20,400))$errors[]='Body weight must be between 20 and 400 kg.';
        $validExercises=0;$validSets=0;
        foreach(($data['exercises']??[]) as $exercise){
            if(empty($exercise['exercise_id']))continue;
            $validExercises++;
            if(($exercise['status']??'completed')==='skipped')continue;
            foreach(($exercise['sets']??[]) as $set){
                $segments=array_values(array_filter($set['segments']??[],fn($segment)=>(($segment['weight_kg']??'')!==''||($segment['repetitions']??'')!=='')));
                if(!$segments&&(($set['weight_kg']??'')!==''||($set['repetitions']??'')!==''))$segments=[['weight_kg'=>$set['weight_kg']??'','repetitions'=>$set['repetitions']??'']];
                if(!$segments)continue;
                $validSets++;
                foreach($segments as $segment){
                    if(($segment['weight_kg']??'')===''||!self::number($segment['weight_kg'],0,2000))$errors[]='A weight value is invalid.';
                    if(($segment['repetitions']??'')===''||!self::integer($segment['repetitions'],0,1000))$errors[]='A repetition value is invalid.';
                }
                if(($set['rest_seconds']??'')!==''&&!self::integer($set['rest_seconds'],0,3600))$errors[]='A rest time is invalid.';
            }
        }
        if($validExercises===0)$errors[]='Add at least one exercise.';
        if($validSets===0)$errors[]='Add at least one completed set.';
