...
final class AchievementService
{
    ...
    private function value(array $definition,?int $sessionId): float
    {
        ...
        if($type==='pr_count')return (float)$db->query("SELECT COUNT(*) FROM exercise_set_segments ss JOIN exercise_sets es ON es.id=ss.exercise_set_id JOIN workout_exercises we ON we.id=es.workout_exercise_id JOIN workout_sessions ws ON ws.id=we.workout_session_id WHERE ss.is_personal_record=1 AND es.set_type='working' AND es.completed=1 AND we.status='completed' AND ws.status='completed'")->fetchColumn();
        if($type==='body_weight_count')return (float)$db->query("SELECT (SELECT COUNT(*) FROM measurements WHERE weight_kg IS NOT NULL)+(SELECT COUNT(*) FROM workout_sessions WHERE body_weight_kg IS NOT NULL AND status='completed')")->fetchColumn();
        ...
        if(in_array($type,['exercise_weight','weight_reps'],true)){$stmt=$db->prepare("SELECT COALESCE(MAX(CASE WHEN e.load_semantics='per_dumbbell' AND COALESCE(we.load_semantics,e.load_semantics)='total' THEN ss.weight_kg/2 ELSE ss.weight_kg END),0) FROM exercise_set_segments ss JOIN exercise_sets es ON es.id=ss.exercise_set_id JOIN workout_exercises we ON we.id=es.workout_exercise_id JOIN exercises e ON e.id=we.exercise_id JOIN workout_sessions ws ON ws.id=we.workout_session_id WHERE we.exercise_id=? AND ws.status='completed' AND we.status='completed' AND es.completed=1 AND es.set_type='working'");$stmt->execute([$exercise]);return (float)$stmt->fetchColumn();}
        if($type==='exercise_reps'){$stmt=$db->prepare("SELECT COALESCE(MAX(ss.repetitions),0) FROM exercise_set_segments ss JOIN exercise_sets es ON es.id=ss.exercise_set_id JOIN workout_exercises we ON we.id=es.workout_exercise_id JOIN workout_sessions ws ON ws.id=we.workout_session_id WHERE we.exercise_id=? AND ws.status='completed' AND we.status='completed' AND es.completed=1 AND es.set_type='working'");$stmt->execute([$exercise]);return (float)$stmt->fetchColumn();}
        if($type==='exercise_complete')return (float)$db->query("SELECT COUNT(*) FROM workout_exercises we JOIN workout_sessions ws ON ws.id=we.workout_session_id WHERE we.status='completed' AND ws.status='completed'")->fetchColumn();
        ...
        if($type==='pr_in_workout'&&$sessionId){$stmt=$db->prepare("SELECT (SELECT COUNT(*) FROM exercise_set_segments ss JOIN exercise_sets es ON es.id=ss.exercise_set_id JOIN workout_exercises we ON we.id=es.workout_exercise_id WHERE we.workout_session_id=? AND ss.is_personal_record=1)+(SELECT COUNT(*) FROM exercise_sets es JOIN workout_exercises we ON we.id=es.workout_exercise_id WHERE we.workout_session_id=? AND es.is_personal_record=1 AND NOT EXISTS(SELECT 1 FROM exercise_set_segments ss WHERE ss.exercise_set_id=es.id))");$stmt->execute([$sessionId,$sessionId]);return (float)$stmt->fetchColumn();}
        if(in_array($type,['sets_top_range','sets_min_reps'],true)){$stmt=$db->prepare("SELECT COALESCE(MAX(set_total),0) FROM (SELECT COUNT(*) set_total FROM exercise_sets es JOIN workout_exercises we ON we.id=es.workout_exercise_id JOIN workout_sessions ws ON ws.id=we.workout_session_id WHERE we.exercise_id=? AND ws.status='completed' AND we.status='completed' AND es.set_type='working' AND es.completed=1 GROUP BY we.id) totals");$stmt->execute([$exercise]);return (float)$stmt->fetchColumn();}
        return 0;
    }
}
