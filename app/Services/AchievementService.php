...
final class AchievementService
{
    ...
    private function value(array $definition,?int $sessionId): float
    {
        ...
        if($type==='pr_count')return (float)$db->query("SELECT (SELECT COUNT(*) FROM exercise_set_segments WHERE is_personal_record=1)+(SELECT COUNT(*) FROM exercise_sets es WHERE es.is_personal_record=1 AND NOT EXISTS(SELECT 1 FROM exercise_set_segments ss WHERE ss.exercise_set_id=es.id))")->fetchColumn();
        ...
        if(in_array($type,['exercise_weight','weight_reps'],true)){$stmt=$db->prepare("SELECT COALESCE(MAX(es.weight_kg),0) FROM exercise_sets es JOIN workout_exercises we ON we.id=es.workout_exercise_id WHERE we.exercise_id=? AND es.completed=1 AND es.set_type='working'");$stmt->execute([$exercise]);return (float)$stmt->fetchColumn();}
        if($type==='exercise_reps'){$stmt=$db->prepare("SELECT COALESCE(MAX(es.repetitions),0) FROM exercise_sets es JOIN workout_exercises we ON we.id=es.workout_exercise_id WHERE we.exercise_id=? AND es.completed=1 AND es.set_type='working'");$stmt->execute([$exercise]);return (float)$stmt->fetchColumn();}
