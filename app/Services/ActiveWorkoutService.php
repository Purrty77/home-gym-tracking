...
final class ActiveWorkoutService
{
    ...
    public function start(int $templateId): array
    {
        ...
        try{
            ...
            $insertExercise=$db->prepare("INSERT INTO workout_exercises(workout_session_id,exercise_id,position,notes,load_semantics,status,target_set_count,target_repetitions_min,target_repetitions_max,planned_rest_seconds,warmup_decision,started_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)");
            ...
            foreach($template['exercises'] as $index=>$exercise){
                ...
                $insertExercise->execute([$sessionId,$exercise['exercise_id'],$exercise['position'],$exercise['notes'],$exercise['load_semantics'],$status,$exercise['set_count'],$exercise['repetitions_min'],$exercise['repetitions_max'],$exercise['rest_seconds'],$decision,$index===0?date('Y-m-d H:i:s'):null]);
    ...
    public function state(int $sessionId): array
    {
        ...
        $stmt=$db->prepare("SELECT we.*,COALESCE(we.load_semantics,e.load_semantics) effective_load_semantics,e.name,e.muscle_group_id,mg.name muscle_group,eq.name equipment,e.weight_increment,e.quick_repetition_values,e.tracking_metric,e.instructions,e.instruction_steps,e.image_path,e.gif_path,e.media_attribution,e.target_muscle,e.secondary_muscles FROM workout_exercises we JOIN exercises e ON e.id=we.exercise_id JOIN muscle_groups mg ON mg.id=e.muscle_group_id LEFT JOIN equipment eq ON eq.id=e.equipment_id WHERE we.workout_session_id=? ORDER BY we.position");
        ...
        if($current){
            ...
            if(!$completedSets){
                $stmt=$db->prepare("SELECT e.id,e.name,e.image_path,e.target_muscle,eq.name equipment,e.recommended_rest_seconds,e.load_semantics FROM exercises e LEFT JOIN equipment eq ON eq.id=e.equipment_id WHERE e.is_active=1 AND e.muscle_group_id=? AND e.id<>? AND NOT EXISTS(SELECT 1 FROM workout_exercises used WHERE used.workout_session_id=? AND used.exercise_id=e.id) ORDER BY e.name");
    ...
    public function completeSet(array $data): array
    {
        ...
        $recordFlags=$type==='working'?$this->personalRecordFlags((int)$current['exercise_id'],(int)$session['id'],$segments,$current['equipment']==='Dumbbells',$current['effective_load_semantics']):array_fill(0,count($segments),false);$isRecord=in_array(true,$recordFlags,true);
        $db->beginTransaction();
        try{
    ...
    public function replaceExercise(int $exerciseId): array
    {
        $state=$this->active();if(!$state||!$state['current'])throw new RuntimeException('No active exercise.');if($state['completedSets'])throw new RuntimeException('The exercise cannot be changed after completing a set.');$candidate=null;foreach($state['replacementCandidates'] as $item)if((int)$item['id']===$exerciseId)$candidate=$item;if(!$candidate)throw new RuntimeException('Choose an available exercise from the same category.');$db=Database::connection();$policy=$this->warmupPolicy($db);$decision=['ask'=>'pending','add'=>'added','never'=>'skipped'][$policy];$db->beginTransaction();try{
            ...
            $db->prepare('UPDATE workout_exercises SET exercise_id=?,load_semantics=?,planned_rest_seconds=?,warmup_decision=?,current_set_position=1,rest_ends_at=NULL,rest_paused_seconds=NULL WHERE id=?')->execute([$exerciseId,$candidate['load_semantics'],$candidate['recommended_rest_seconds'],$decision,$state['current']['id']]);$db->prepare('UPDATE exercise_sets SET weight_kg=NULL,repetitions=NULL,rest_seconds=? WHERE workout_exercise_id=? AND completed=0')->execute([$candidate['recommended_rest_seconds'],$state['current']['id']]);$db->prepare('UPDATE workout_sessions SET current_set_position=1 WHERE id=?')->execute([$state['session']['id']]);$db->commit();return ['status'=>'replaced','exercise'=>$candidate['name'],'warmup'=>$decision];
        }catch(Throwable $e){$db->rollBack();throw $e;}
    }
    ...
    private function personalRecordFlags(int $exerciseId,int $sessionId,array $segments,bool $isDumbbell,string $mode): array
    {
        $stmt=Database::connection()->prepare("SELECT CASE WHEN eq.name='Dumbbells' AND COALESCE(we.load_semantics,e.load_semantics)='total' THEN es.weight_kg/2 ELSE es.weight_kg END weight_kg,es.repetitions FROM exercise_sets es JOIN workout_exercises we ON we.id=es.workout_exercise_id JOIN exercises e ON e.id=we.exercise_id LEFT JOIN equipment eq ON eq.id=e.equipment_id JOIN workout_sessions ws ON ws.id=we.workout_session_id WHERE we.exercise_id=? AND es.set_type='working' AND es.completed=1 AND (ws.status='completed' OR ws.id=?) ORDER BY weight_kg DESC,es.repetitions DESC LIMIT 1");$stmt->execute([$exerciseId,$sessionId]);$best=$stmt->fetch();return array_map(function($segment)use($best,$isDumbbell,$mode){$candidate=$segment;if($isDumbbell&&$mode==='total')$candidate['weight_kg']=(float)$candidate['weight_kg']/2;return !$best||$this->comparePerformance($candidate,$best)>0;},$segments);
    }
