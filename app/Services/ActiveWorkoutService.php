...
final class ActiveWorkoutService
{
    ...
    public function start(int $templateId): array
    {
        ...
        try{$weight=$db->query("SELECT weight_kg FROM measurements WHERE measured_on=CURRENT_DATE AND weight_kg IS NOT NULL UNION ALL SELECT body_weight_kg FROM workout_sessions WHERE DATE(performed_at)=CURRENT_DATE AND body_weight_kg IS NOT NULL LIMIT 1")->fetchColumn()?:null;$stmt=$db->prepare("INSERT INTO workout_sessions(workout_template_id,performed_at,session_type,body_weight_kg,status,current_set_position,started_at) VALUES(?,NOW(),?,?, 'in_progress',1,NOW())");$stmt->execute([$templateId,$template['session_type'],$weight]);$sessionId=(int)$db->lastInsertId();$insert=$db->prepare("INSERT INTO workout_exercises(workout_session_id,exercise_id,position,notes,status,target_set_count,target_repetitions_min,target_repetitions_max,planned_rest_seconds,started_at) VALUES(?,?,?,?,?,?,?,?,?,?)");$first=null;foreach($template['exercises'] as $index=>$exercise){$status=$index===0?'active':'pending';$insert->execute([$sessionId,$exercise['exercise_id'],$exercise['position'],$exercise['notes'],$status,$exercise['set_count'],$exercise['repetitions_min'],$exercise['repetitions_max'],$exercise['rest_seconds'],$index===0?date('Y-m-d H:i:s'):null]);if($index===0)$first=(int)$db->lastInsertId();}$db->prepare('UPDATE workout_sessions SET current_workout_exercise_id=? WHERE id=?')->execute([$first,$sessionId]);$db->commit();return $this->state($sessionId);}catch(Throwable $e){$db->rollBack();throw $e;}
    }
    ...
    public function state(int $sessionId): array
    {
        ...
        return compact('session','exercises','current','previous','completedSets','defaultWeight','remaining','settings');
    }

    public function completeSet(array $data): array
    {
        $state=$this->active();if(!$state||!$state['current'])throw new RuntimeException('No active set.');$weight=$data['weight_kg']??null;$reps=$data['repetitions']??null;if(!is_numeric($weight)||!is_numeric($reps)||(float)$weight<0||(int)$reps<0)throw new RuntimeException('Enter a valid weight and repetition count.');$db=Database::connection();$session=$state['session'];$current=$state['current'];$position=(int)$session['current_set_position'];$type=in_array($data['set_type']??'working',['working','warmup','ramp'],true)?$data['set_type']:'working';
        ...
        $stmt=$db->prepare('INSERT INTO exercise_sets(workout_exercise_id,position,set_type,weight_kg,repetitions,rest_seconds,notes,completed,completed_at,is_personal_record) VALUES(?,?,?,?,?,?,?,?,NOW(),?)');$stmt->execute([$current['id'],$position,$type,$weight,$reps,$current['planned_rest_seconds'],trim($data['notes']??'')?:null,1,$isRecord?1:0]);$setId=(int)$db->lastInsertId();$target=(int)$current['target_set_count'];
