...
final class ActiveWorkoutService
{
    ...
    public function state(int $sessionId): array
    {
        ...
        $stmt=$db->prepare("SELECT we.*,e.name,e.muscle_group_id,mg.name muscle_group,e.weight_increment,e.quick_repetition_values,e.tracking_metric,e.instructions,e.instruction_steps,e.image_path,e.gif_path,e.media_attribution,e.target_muscle,e.secondary_muscles FROM workout_exercises we JOIN exercises e ON e.id=we.exercise_id JOIN muscle_groups mg ON mg.id=e.muscle_group_id WHERE we.workout_session_id=? ORDER BY we.position");$stmt->execute([$sessionId]);$exercises=$stmt->fetchAll();$current=null;foreach($exercises as $exercise)if((int)$exercise['id']===(int)$session['current_workout_exercise_id'])$current=$exercise;
        $previous=[];$completedSets=[];$currentSet=null;$extraSets=[];$replacementCandidates=[];$defaultWeight=null;if($current){$stmt=$db->prepare('SELECT * FROM exercise_sets WHERE workout_exercise_id=? ORDER BY position');$stmt->execute([$current['id']]);$allSets=$stmt->fetchAll();$completedSets=array_values(array_filter($allSets,fn($set)=>(int)$set['completed']===1));$extraSets=array_values(array_filter($allSets,fn($set)=>(int)($set['is_extra']??0)===1));foreach($allSets as $set)if((int)$set['position']===(int)$session['current_set_position'])$currentSet=$set;$stmt=$db->prepare("SELECT es.* FROM exercise_sets es JOIN workout_exercises we ON we.id=es.workout_exercise_id JOIN workout_sessions ws ON ws.id=we.workout_session_id WHERE we.exercise_id=? AND ws.status='completed' AND ws.id<>? AND es.set_type='working' AND es.completed=1 AND ws.id=(SELECT MAX(ws2.id) FROM workout_sessions ws2 JOIN workout_exercises we2 ON we2.workout_session_id=ws2.id WHERE we2.exercise_id=? AND ws2.status='completed') ORDER BY es.position");$stmt->execute([$current['exercise_id'],$sessionId,$current['exercise_id']]);$previous=$stmt->fetchAll();$setIndex=max(0,(int)$session['current_set_position']-2);$defaultWeight=$currentSet['weight_kg']??($previous[$setIndex]['weight_kg']??($previous?end($previous)['weight_kg']:null));if(!$completedSets){$stmt=$db->prepare("SELECT e.id,e.name,e.image_path,e.target_muscle,eq.name equipment,e.recommended_rest_seconds FROM exercises e LEFT JOIN equipment eq ON eq.id=e.equipment_id WHERE e.is_active=1 AND e.muscle_group_id=? AND e.id<>? AND NOT EXISTS(SELECT 1 FROM workout_exercises used WHERE used.workout_session_id=? AND used.exercise_id=e.id) ORDER BY e.name");$stmt->execute([$current['muscle_group_id'],$current['exercise_id'],$sessionId]);$replacementCandidates=$stmt->fetchAll();}}
        ...
        return compact('session','exercises','current','currentSet','extraSets','replacementCandidates','previous','completedSets','defaultWeight','remaining','settings');
    }
    ...
    {
        $state=$this->active();if(!$state||!$state['current'])throw new RuntimeException('No active exercise.');if($state['completedSets'])throw new RuntimeException('The exercise cannot be changed after completing a set.');$candidate=null;foreach($state['replacementCandidates'] as $item)if((int)$item['id']===$exerciseId)$candidate=$item;if(!$candidate)throw new RuntimeException('Choose an available exercise from the same category.');$db=Database::connection();$db->beginTransaction();try{$db->prepare('UPDATE workout_exercises SET exercise_id=?,planned_rest_seconds=? WHERE id=?')->execute([$exerciseId,$candidate['recommended_rest_seconds'],$state['current']['id']]);$db->prepare('UPDATE exercise_sets SET weight_kg=NULL,repetitions=NULL,rest_seconds=? WHERE workout_exercise_id=? AND completed=0')->execute([$candidate['recommended_rest_seconds'],$state['current']['id']]);$db->commit();return ['status'=>'replaced','exercise'=>$candidate['name']];}catch(Throwable $e){$db->rollBack();throw $e;}
    }
