...
final class StatisticsService
{
    public function dashboard(): array
    {
        ...
        $week=$db->query("SELECT COUNT(DISTINCT ws.id) sessions,COUNT(CASE WHEN es.set_type='working' AND es.completed=1 AND we.status='completed' THEN 1 END) sets FROM workout_sessions ws LEFT JOIN workout_exercises we ON we.workout_session_id=ws.id LEFT JOIN exercise_sets es ON es.workout_exercise_id=we.id WHERE ws.status='completed' AND YEARWEEK(ws.performed_at,1)=YEARWEEK(CURRENT_DATE,1)")->fetch();
        $records=$db->query("SELECT e.id,e.name,MAX(CASE WHEN eq.name='Dumbbells' AND e.load_semantics='per_dumbbell' AND COALESCE(we.load_semantics,e.load_semantics)='total' THEN es.weight_kg/2 WHEN eq.name='Dumbbells' AND e.load_semantics='total' AND COALESCE(we.load_semantics,e.load_semantics)='per_dumbbell' THEN es.weight_kg*2 ELSE es.weight_kg END) max_weight FROM exercises e LEFT JOIN equipment eq ON eq.id=e.equipment_id JOIN workout_exercises we ON we.exercise_id=e.id JOIN workout_sessions ws ON ws.id=we.workout_session_id JOIN exercise_sets es ON es.workout_exercise_id=we.id WHERE ws.status='completed' AND we.status='completed' AND es.completed=1 AND es.set_type='working' AND es.weight_kg IS NOT NULL GROUP BY e.id,e.name ORDER BY max_weight DESC LIMIT 5")->fetchAll();
        $recordCount=(int)$db->query("SELECT COUNT(DISTINCT we.exercise_id) FROM workout_exercises we JOIN workout_sessions ws ON ws.id=we.workout_session_id JOIN exercise_sets es ON es.workout_exercise_id=we.id WHERE ws.status='completed' AND we.status='completed' AND es.completed=1 AND es.set_type='working' AND es.weight_kg IS NOT NULL")->fetchColumn();
