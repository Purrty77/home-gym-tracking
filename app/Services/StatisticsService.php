...
final class StatisticsService
{
    public function dashboard(): array
    {
        ...
        $weight=$db->query("SELECT label,AVG(value) value FROM (SELECT measured_on label,weight_kg value FROM measurements WHERE weight_kg IS NOT NULL UNION ALL SELECT DATE(performed_at) label,body_weight_kg value FROM workout_sessions WHERE body_weight_kg IS NOT NULL) weights GROUP BY label ORDER BY label")->fetchAll();
        $currentWeight=$weight?(float)end($weight)['value']:null;$previousWeight=count($weight)>1?(float)$weight[count($weight)-2]['value']:null;$entryWeightChange=$currentWeight!==null&&$previousWeight!==null?$currentWeight-$previousWeight:null;$weightChange=null;$weeklyWeightTrend=null;$monthlyWeightTrend=null;
